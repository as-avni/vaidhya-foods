  ...
  return (
    <header className="bg-white bg-opacity-80 backdrop-blur-md shadow-lg sticky md:fixed top-0 w-full px-4 md:px-10 py-3 flex items-center justify-center md:justify-between z-50">
      ...
      <div className="flex-1 flex justify-center">
        <Link href="/" className="absolute left-1/2 transform -translate-x-1/2 md:static">
          <Image 
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden fixed top-0 left-0 h-full w-64 bg-green-50 shadow-lg py-6 px-4 flex flex-col items-start gap-6 transition-transform transform translate-x-0">
          ...
          {["Home", "About Us", "Our Products", "Contact Us"].map((item, index) => (
            <Link 
              key={index} 
              href={`/${item === "Home" ? "" : `#${item.toLowerCase().replace(/\s+/g, "")}`}`} 
              className="relative text-black hover:text-green-600 text-lg font-medium after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-green-600 after:transition-all after:duration-300 hover:after:w-full"
            >
              {item}
            </Link>
          ))}
