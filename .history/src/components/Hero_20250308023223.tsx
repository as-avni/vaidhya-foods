...
export default function Hero() {
  ...
  {backgroundImages.map((image, index) => (
    <div
      key={index}
      className={`absolute inset-0 w-full h-full bg-no-repeat bg-contain bg-center transition-opacity duration-1000 ${
  ...
  <div className="container md:mt-0 mt-8 relative z-10 flex flex-col items-start justify-center h-full text-white">
    <div className="max-w-2xl">
      <h1 className="mb-4 text-xl font-bold font-sriracha leading-tight md:text-6xl">
      ...
      <div className="flex flex-wrap gap-4">
        <Link href="#products" className="bg-green-300 text-black btn  text-sm md:text-lg rounded-full hover:bg-white">
          What We Serve
        </Link>
        <button
          onClick={() => setIsModalOpen(true)}
          className="btn text-sm md:text-lg text-black bg-green-300 rounded-full flex flex-wrap gap-2 hover:bg-white"
        >          
          <PlayCircle className="md:h-6 md:w-6  h-4 w-4 text-black" strokeWidth={1.5} />
          View Our Product
        </button>
      </div>
    </div>
  ...
  {/* Indicators */}
  <div className="hidden absolute bottom-8 left-0 right-0 z-10 md:flex justify-center">
