...
//       image: "/img/testimonials/testimonials-2.png",
//       rating: 5,
...
//       name: "Priya Sharma",
//       position: "Home Chef",
//       image: "/img/testimonials/testimonials-1.png",
//       rating: 5,
...
//                       <div className="relative overflow-hidden rounded-full md:w-44 md:h-44 h-72 w-72 mt-16 md:mt-0">
//                         <Image
//                           src={review.image || "/"}
//                           alt={review.name}
//                           fill
...
export default function Reviews() {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval)
  }, [reviews.length]) // ✅ Added dependency
  ...
  return (
    <section className="py-20 bg-gray-50">
      <div className="container">
        <h2 className="section-title font-sriracha text-center text-3xl md:text-4xl font-bold">
          What Are They Saying About Us
        </h2>

        <div className="max-w-4xl mx-auto mt-8">
          <div className="relative overflow-hidden bg-white rounded-lg shadow-lg border-l-4 border-green-500">
            <div
              className="flex transition-transform duration-500 ease-in-out"
              style={{ transform: `translateX(-${currentReviewIndex * 100}%)`, width: `${reviews.length * 100}%` }}
            >
              {reviews.map((review) => (
                <div key={review.id} className="flex-shrink-0 w-full p-8 md:p-12">
                  <div className="flex flex-col items-center md:flex-row">
                    
                    {/* Image Section */}
                    <div className="relative w-32 h-32 md:w-44 md:h-44 mb-6 md:mb-0 md:mr-6">
                      <Image
                        src={review.image}
                        alt={review.name}
                        fill
                        className="object-cover rounded-full border border-gray-300 shadow-lg"
                      />
                    </div>

                    {/* Text Section */}
                    <div className="text-center md:text-left">
                      <p className="mb-4 text-lg italic text-gray-800">"{review.text}"</p>
                      ...
                      <p className="text-green-600 font-medium">{review.position}</p>
                    </div>
            ...
            {reviews.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  index === currentReviewIndex ? "bg-green-600 w-4 h-4" : "bg-gray-300"
                }`}
                aria-label={`Go to review ${index + 1}`}
              />
            ))}
