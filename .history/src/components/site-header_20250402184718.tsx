    ...
    <header className="bg-white bg-opacity-80 backdrop-blur-md shadow-lg sticky top-0 w-full px-4 md:px-10 py-3 flex items-center justify-between z-50">
      ...
      <nav className="hidden md:flex gap-8">
        {["Home", "About Us", "Our Products", "Contact Us"].map((item, index) => (
          <Link
            ...
            className="text-black hover:text-red-600 text-sm font-medium transition-all duration-300"
          >
            {item}
          </Link>
        ))}
      ...
      <div className="hidden md:flex">
        <Link
          ...
          className="bg-black text-sm text-white px-4 py-2 rounded-full font-medium hover:bg-green-300 hover:bg-green-300 transition-all duration-300"
        >
          Call Us
        </Link>
      </div>
