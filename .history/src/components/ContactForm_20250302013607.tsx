  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <textarea name="message" placeholder="Your Message" rows={6} value={formData.message} onChange={handleChange} className="w-full px-4 py-2 border rounded-md border-gray-300 focus:outline-none focus:ring-2 focus:ring-green-500" required></textarea>
        <div className="text-center">
          <button type="submit" disabled={isSubmitting} className="px-8 py-3 font-medium text-black transition-colors rounded-2xl bg-green-100 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-50 focus:ring-offset-2">
            {isSubmitting ? "Sending..." : "Send"}
          </button>
        </div>
      </form>
