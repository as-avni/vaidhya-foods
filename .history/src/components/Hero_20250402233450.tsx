...
export default function Hero() {
  ...
  const backgroundImages = [
    ...
    "https://res.cloudinary.com/dhi3iupue/image/upload/v1743614571/kadhi_dahi_wrripz.png",
  ];
  ...
  {backgroundImages.map((image, index) => (
    <div
      key={index}
      className={`absolute inset-0 w-full h-full bg-[length:500%] md:bg-[length:120%] sm:bg-center transition-opacity duration-1000 ${
  ...
  {/* Content */}
  <div className="container top-0 md:top-0 md:mt-0 relative z-10 flex flex-col items-start justify-center h-full text-white">
    <div className="max-w-2xl">
      <h1 className="mt-20 mb-4 text-5xl font-bold font-sriracha text-center leading-tight md:text-6xl">
        Taste of Purity
      </h1>
      <p className="mb-8 text-xl font-sriracha text-center">Creamy, Dreamy, And Always Fresh</p>
      <div className="flex flex-wrap gap-4 items-center justify-center">
        <Link href="#products" className="bg-green-300 text-black btn rounded-full hover:bg-white w-auto text-sm">
          What We Serve
        </Link>
        <button
          onClick={() => setIsModalOpen(true)}
          className="btn text-black bg-green-300 rounded-full flex flex-wrap gap-2 hover:bg-white text-sm"
        >          
          <PlayCircle className="h-4 w-6 text-black" strokeWidth={1.5} />
          View Our Product
        </button>
      </div>
    </div>
