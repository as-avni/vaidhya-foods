...
export default function Hero() {
  ...
  {backgroundImages.map((image, index) => (
    <div
      key={index}
      className={`absolute inset-0 w-full h-screen sm:h-full bg-[length:120%] sm:bg-cover bg-center transition-opacity duration-1000 ${
  ...
  {/* Content */}
  <div className="container md:mt-0 mt-8 relative z-10 flex flex-col items-center text-center justify-center h-full text-white">
    <div className="max-w-2xl">
      ...
      <div className="flex flex-wrap gap-4">
        <Link href="#products" className="bg-green-300 text-black btn rounded-full hover:bg-white px-4 py-2 text-sm">
          What We Serve
        </Link>
        <button
          onClick={() => setIsModalOpen(true)}
          className="btn text-black bg-green-300 rounded-full flex flex-wrap gap-2 hover:bg-white px-4 py-2 text-sm"
        >          
